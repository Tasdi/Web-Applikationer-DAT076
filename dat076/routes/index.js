...
/*Function to ensure that admin cannot route to page of a client */
...
/*Function to ensure that patient cannot route to page of an admin */
...
/******************************************************************************
** Post method to register a user. Uses validation is used to check to see if
** all required fields are filled. Checks whether user already exist in the
** databse i.e. registered, if the password type in for a user is correct, etc.
*******************************************************************************/
...
router.post('/createBooking', function(req, res){
	...
	if(errors || startTime == endTime || endTime < startTime){
		...
		//Difference between startTime and endTime is stored in result. This is done
		//so we can divide the time specified by the admin (e.g. 08:00-9:00) and create
		//several bookings out of it (in this case 08-08:30 and 08:30-09:00).
		var result = 
			(parseFloat(endTime.split(':')[0])) + (parseFloat(endTime.split(':')[1]) /60) - (parseFloat(startTime.split(':')[0])) + (parseFloat(startTime.split(':')[1] /60));
		...
		req.flash('success_msg', 'Your bookings are stored');
		res.redirect('/admin');
	}
});
